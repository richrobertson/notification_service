...
func main() {
	run("webhook-worker", queue.DispatchWebhookQueueName, func(cfg config.Config, postgres *store.Postgres, redisQueue *queue.RedisQueue) (*delivery.Service, error) {
		...
		return delivery.NewService(postgres, webhookSender, webhookSender, delivery.NewSMTPSender(cfg), delivery.NewSecondarySMTPSender(cfg), delivery.RetryPolicy{MaxAttempts: cfg.RetryMaxAttempts, BaseDelay: cfg.RetryBaseDelay, MaxDelay: cfg.RetryMaxDelay, ExponentialBackoff: cfg.RetryExponentialBackoff, Jitter: cfg.RetryJitter, Now: func() time.Time { return time.Now().UTC() }, PressureMultiplier: cfg.RetryPressureMultiplier, PressureMinDelay: cfg.RetryPressureMinDelay, QueueSoftLimit: cfg.QueueSoftLimit, QueueDepth: func(channel string) int {
