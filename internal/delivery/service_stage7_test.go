...
func TestRetryDelayExpandsUnderPressure(t *testing.T) {
	...
	svc, err := NewService(st, stubWebhookSender{}, stubWebhookSender{}, stubEmailSender{err: &RetryableError{Err: errors.New("smtp down")}}, stubEmailSender{}, RetryPolicy{MaxAttempts: 3, BaseDelay: 5 * time.Second, MaxDelay: time.Minute, ExponentialBackoff: true, Now: func() time.Time { return time.Unix(10, 0).UTC() }, IDGenerator: func() string { return "dead-1" }, RandSource: rand.New(rand.NewSource(1)), QueueSoftLimit: 10, PressureMultiplier: 2, PressureMinDelay: 20 * time.Second, QueueDepth: func(string) int { return 10 }})
	if err != nil {
		t.Fatal(err)
	}
